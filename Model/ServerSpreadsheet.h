...
class Cell {
	...
	std::stack<Cell*>* history; // History of the cell
...
public:
	...
	std::stack<Cell*>* get_history(); // Get the revert history of this cell
	void pop_history(); // Pop off the top of history of this cell
...
class Spreadsheet {
	...
	boost::json::array get_json_cell_history(Cell c);	// Gets the history stack of a cell from the file
...
public:
	...
	bool set_cell(std::string cell_name, std::string contents);		// Sets a cell, given by cell name, to the given contents. Returns true if no errors occurred, false if 
